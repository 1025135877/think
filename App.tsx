...
import { generateMystery, judgeInput, evaluateSolution, setApiKey } from './services/geminiService';
...
function App() {
  ...
  useEffect(() => {
    const checkApiKey = async () => {
      if (window.aistudio) {
        ...
        setApiKey(process.env.GEMINI_API_KEY || process.env.API_KEY || "");
        setHasApiKey(true);
      }
    };
    checkApiKey();
  ...
  const handleSelectKey = async () => {
    if (window.aistudio) {
      ...
      const manualKey = prompt("系统检测到不在专用环境，请输入您的 Gemini API Key：");
      if (manualKey) {
        setApiKey(manualKey);
        setHasApiKey(true);
      } else {
        alert("需要 API Key 才能建立神经连接。");
      }
    }
  };
  ...
  const startNewGame = useCallback(async () => {
    ...
    try {
      console.log("Starting new game, generating mystery...");
      const data = await generateMystery();
      console.log("Mystery generated successfully:", data);
      setMystery(data);
      const initMessage = {
        id: 'init-situation',
      ...
      console.log("Setting initial message:", initMessage);
      setMessages([initMessage]);
      setPhase(GamePhase.PLAYING);
    } catch (error: any) {
      console.error("Game creation failed:", error);
      const isKeyError = error.message === "KEY_RESELECT_REQUIRED" ||
      ...
      if (isKeyError) {
        setHasApiKey(false);
        alert("API 密钥校验失败或权限不足，请重新设置。");
      } else {
        alert(`卷宗生成失败: ${error.message || '未知错误'}`);
      }
              ...
              {mystery.npcs.map(npc => {
                ...
                return (
                  <button
                    key={npc.id}
                    disabled={isDeceased}
                    onClick={() => { if (!isDeceased) { setActiveTarget(npc.id); setShowCluePanel(false); } }}
                    className={`w-full text-left p-3 rounded-2xl border transition-all flex items-center gap-4 relative overflow-hidden group
                              ${isDeceased ? 'opacity-40 grayscale cursor-not-allowed border-rose-900/20 bg-rose-950/5' : activeTarget === npc.id ? 'bg-indigo-600/10 border-indigo-500/50 shadow-xl' : 'bg-slate-900/20 border-white/5 text-slate-500 hover:bg-slate-900 hover:border-white/10'}`}
                  >
                    <div className="w-12 h-12 rounded-xl bg-slate-950 flex-shrink-0 flex items-center justify-center overflow-hidden border border-white/10 z-10 shadow-inner group-hover:scale-105 transition-transform duration-500">
                      {npc.avatarUrl ? (
                        <img src={npc.avatarUrl} alt={npc.name} className={`w-full h-full object-cover transition-all ${isDeceased ? 'grayscale' : 'grayscale-[0.3] group-hover:grayscale-0'}`} />
                      ) : (
                        <span className="text-xs font-bold text-white/30">{npc.name.charAt(0)}</span>
                      )}
                      {isDeceased && <div className="absolute inset-0 bg-rose-950/20 flex items-center justify-center text-[8px] font-black text-rose-500 uppercase rotate-12">Closed</div>}
                    </div>
                    <div className="z-10 min-w-0">
                      <div className={`font-black text-xs tracking-tight transition-colors ${isDeceased ? 'text-rose-500/50' : activeTarget === npc.id ? 'text-white' : 'text-slate-400'}`}>
                        {npc.name}
                        {isDeceased && <span className="ml-2 text-[8px] text-rose-500/80 border border-rose-500/30 px-1 rounded">死者</span>}
                      </div>
                      <div className="text-[9px] opacity-50 font-serif italic truncate mt-0.5">{npc.role}</div>
                    </div>
                    {activeTarget === npc.id && !isDeceased && (
